...
C
      ...
      IMPLICIT NONE
C
      INTEGER N1,N2,N3,I,ID,IP,NN3
      INTEGER NSAM,K,N,NN,ITEMP,NF,NX,MODE
      INTEGER(KIND=8) NSAMH3
      INTEGER JC,L,LL,M,MM,NSAMH,KK,K2
      REAL PSIZE,F11,F22,X11,X22
      REAL PI,PDIFF,SUM,PDK,E,W
      PARAMETER (PI=3.1415926535897)
C     Allocatable arrays sized at runtime for large map support
C     DATA1/DATA2 are REAL for I/O; CDATA1/CDATA2 are COMPLEX views for FFT/spectral ops
      REAL,    ALLOCATABLE :: DATA1(:),DATA2(:)
      COMPLEX, ALLOCATABLE :: CDATA1(:),CDATA2(:)
      COMPLEX, ALLOCATABLE :: SPEQ1(:),SPEQ2(:)
      COMPLEX, ALLOCATABLE :: F1LCL(:),F2LCL(:)
      REAL,    ALLOCATABLE :: IN(:),RF(:),SCAL(:),PD(:)
      INTEGER, ALLOCATABLE :: NXF(:)
      CHARACTER FNAME*80,CFORM,VX*15,TITLE*80
C
      DATA  VX/'1.13 - 26.06.21'/
C     15 chars 'X.XX - XX.XX.XX' <--<--<--<--<--<--<--<--
C
      WRITE(*,*) 'OpenMP threads available:', OMP_GET_MAX_THREADS()
      WRITE(*,1010)VX
1010  FORMAT('  DIFFMAP ',A15,
     +      /'  Distributed under the GNU',
     +       ' General Public License (GPL)'//,
     +       '  Output map = Input 1 - Input 2',/)
C
      WRITE(*,*)'Image format [M,S,I]?'
      READ(*,1100)CFORM
1100  FORMAT(A1)
      WRITE(*,*)'First input 2D or 3D map?'
      READ(*,1000)FNAME
1000  FORMAT(A80)
     ...
     .  STOP 'X,Y,Z dimensions not equal or Z dimension not 1'
      ...
      NSAM=N1
C
C     Allocate arrays based on actual map dimensions
      ALLOCATE(DATA1(NSAM*NSAM*N3))
      ALLOCATE(DATA2(NSAM*NSAM*N3))
      ALLOCATE(CDATA1(NSAM*NSAM*N3/2))
      ALLOCATE(CDATA2(NSAM*NSAM*N3/2))
      ALLOCATE(SPEQ1(NSAM*N3))
      ALLOCATE(SPEQ2(NSAM*N3))
      ALLOCATE(IN(NSAM/2))
      ALLOCATE(RF(NSAM/2))
      ALLOCATE(SCAL(NSAM/2))
      ALLOCATE(PD(NSAM/2))
      ALLOCATE(NXF(NSAM/2))
      NSAMH=NSAM/2
      NSAMH3=INT(NSAM,KIND=8)*INT(N3,KIND=8)*INT(NSAMH,KIND=8)
      JC=NSAM/2+1
C
      WRITE(*,*)'Second input 2D or 3D map?'
      READ(*,1000)FNAME
      CALL IOPEN(FNAME,11,CFORM,MODE,N1,N2,NN3,'OLD',
     .           PSIZE,TITLE)
      IF ((N1.NE.NSAM).OR.(N2.NE.NSAM).OR.(NN3.NE.N3))
     .  STOP 'X,Y,Z dimensions not equal to input 1'
C
...
C
      DO 10 K=1,N3
        ...
        DO 10 I=1,NSAM
          ...
          CALL IREAD(11,DATA2(ID),I+IP)
10    CONTINUE
C
      CALL ICLOSE(10)
      CALL ICLOSE(11)
C
      WRITE(*,*)'Output diffmap?'
      READ(*,1000)FNAME
C
      TITLE='DIFFMAP: Difference map MAP1 - scaled MAP2'
      CALL IOPEN(FNAME,10,CFORM,MODE,N1,N2,N3,'NEW',
     .           PSIZE,TITLE)
C
      WRITE(*,*)'Output scaled second input 3D map?'
      READ(*,1000)FNAME
C
      TITLE='DIFFMAP: Scaled MAP2'
      CALL IOPEN(FNAME,11,CFORM,MODE,N1,N2,N3,'NEW',
     .           PSIZE,TITLE)
C
C     Copy real data into complex arrays for FFT
      CDATA1 = CMPLX(DATA1(1::2), DATA1(2::2))
      CDATA2 = CMPLX(DATA2(1::2), DATA2(2::2))
      CALL RLFT3(CDATA1,SPEQ1,NSAM,NSAM,N3,1)
      CALL RLFT3(CDATA2,SPEQ2,NSAM,NSAM,N3,1)
!$OMP PARALLEL DO SCHEDULE(STATIC)
      DO 39 K=1,NSAM*N3
        SPEQ1(K)=CMPLX(0.0,0.0)
        SPEQ2(K)=CMPLX(0.0,0.0)
39    CONTINUE
!$OMP END PARALLEL DO
C
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1)
!$OMP+  PRIVATE(K2,KK,NX,L,LL,M,MM,N,NN,ITEMP,ID,NF,E,X11,F11,X22,F22)
!$OMP+  PRIVATE(F1LCL,F2LCL)
      DO 40 K=1,NSAM/2-1
        K2=K**2
        KK=(K+1)**2
        NX=0
        ALLOCATE(F1LCL(NSAM*NSAM*N3/2))
        ALLOCATE(F2LCL(NSAM*NSAM*N3/2))
        DO L=1,JC-1
          LL=L-1
          DO M=1,NSAM
            DO N=1,N3
              ...
              IF ((ITEMP.GE.K2).AND.(ITEMP.LT.KK)) THEN
                NX=NX+1
        ...
        X11=1.0
        F11=1.0
        CALL VA04A(NX,F1LCL,F2LCL,IN(K),X11,E,NF,F11,100.0,0,1,50)
        X22=1.0
        F22=1.0
        CALL VA04A(NX,F2LCL,F1LCL,IN(K),X22,E,NF,F22,100.0,0,1,50)
        SCAL(K)=SQRT(ABS(X11))/SQRT(ABS(X22))
        CALL CALCFX(NF,SCAL(K),F11,NX,F1LCL,F2LCL,IN(K))
        RF(K)=F11
        NXF(K)=NX
        DEALLOCATE(F1LCL,F2LCL)
40    CONTINUE
!$OMP END PARALLEL DO
C
      CDATA2(1)=CDATA1(1)/NSAMH3
!$OMP PARALLEL DO SCHEDULE(DYNAMIC,1)
!$OMP+  PRIVATE(K2,KK,L,LL,M,MM,N,NN,ITEMP,ID,W,PDK,SUM)
      DO 50 K=1,NSAM/2-1
        PDK=0.0
        SUM=0.0
        K2=K**2
        ...
        DO L=1,JC-1
          DO M=1,NSAM
            DO N=1,N3
              ...
              IF ((ITEMP.GE.K2).AND.(ITEMP.LT.KK)) THEN
                ID=L+NSAMH*((M-1)+NSAM*(N-1))
                CDATA2(ID)=CDATA2(ID)*SCAL(K)
                W=CABS(CDATA1(ID))*CABS(CDATA2(ID))
                PDK=PDK+PDIFF(CDATA1(ID),CDATA2(ID))*W
                SUM=SUM+W
                CDATA1(ID)=CDATA1(ID)-CDATA2(ID)
                CDATA1(ID)=CDATA1(ID)/NSAMH3
                CDATA2(ID)=CDATA2(ID)/NSAMH3
              ENDIF
            END DO
          END DO
        END DO
        PD(K)=PDK/SUM/PI*180.0
50    CONTINUE
!$OMP END PARALLEL DO
C
      CDATA1(1)=CMPLX(0.0,0.0)
        KK=(NSAM/2)**2
!$OMP PARALLEL DO COLLAPSE(3) SCHEDULE(STATIC)
!$OMP+  PRIVATE(LL,MM,NN,ITEMP,ID)
        DO L=1,JC-1
          DO M=1,NSAM
            DO N=1,N3
              LL=L-1
              ...
              IF (ITEMP.GE.KK) THEN
                ID=L+NSAMH*((M-1)+NSAM*(N-1))
                CDATA1(ID)=CMPLX(0.0,0.0)
                CDATA2(ID)=CMPLX(0.0,0.0)
              ENDIF
            END DO
          END DO
        END DO
...
C
      PRINT *
      WRITE(*,*)'Resolution  Weighting factor  R-factor  Phase  ',
     +          '         Amplitude      Npix'
      WRITE(*,*)'              for input 2             residual ',
     +          '       for input 1'
      DO 30 I=1,NSAM/2-1
        WRITE(*,1003)REAL(NSAM)/I*PSIZE,SCAL(I),RF(I),PD(I),
     +               SQRT(IN(I)),NXF(I)
1003    FORMAT(F11.2,F18.4,F10.6,F7.2,F20.6,I10)
30    CONTINUE
      PRINT *
C
      CALL RLFT3(CDATA1,SPEQ1,NSAM,NSAM,N3,-1)
      CALL RLFT3(CDATA2,SPEQ2,NSAM,NSAM,N3,-1)
C     Copy complex results back to real arrays for I/O
      DATA1(1::2) = REAL(CDATA1)
      DATA1(2::2) = AIMAG(CDATA1)
      DATA2(1::2) = REAL(CDATA2)
      DATA2(2::2) = AIMAG(CDATA2)
C
      DO 20 K=1,N3
        IP=NSAM*(K-1)
        DO 20 I=1,NSAM
          ID=1+NSAM*((I-1)+NSAM*(K-1))
          CALL IWRITE(10,DATA1(ID),I+IP)
          CALL IWRITE(11,DATA2(ID),I+IP)
20    CONTINUE
C
      CALL ICLOSE(10)
      CALL ICLOSE(11)
      PRINT *
      DEALLOCATE(DATA1,DATA2,CDATA1,CDATA2,SPEQ1,SPEQ2)
      DEALLOCATE(IN,RF,SCAL,PD,NXF)
      STOP ' NORMAL TERMINATION OF DIFFMAP'
C
      END
C
...
C**************************************************************************
C     Phase calculation done if complex number non-zero
...
C**************************************************************************
      ...
      END
C**************************************************************************
      SUBROUTINE CALCFX(N,X,F,NX,F1,F2,IN)
C**************************************************************************
      IMPLICIT NONE
C
      INTEGER N,NX,I
      REAL X(*),F,SUM1,SUM2,IN
      COMPLEX F1(*),F2(*)
C
      SUM1=0.0
      SUM2=0.0
      DO 10 I=1,NX
        SUM1=SUM1+(CABS(F1(I))-CABS(X(1)*F2(I)))**2
C        SUM1=SUM1+2.0*ABS(CABS(F1(I))-CABS(X(1)*F2(I))) 
        SUM2=SUM2+CABS(F1(I))**2
C        SUM2=SUM2+CABS(F1(I))+CABS(X(1)*F2(I))
10    CONTINUE
C      F=1.0
C      F=SUM1/NX
      IF (SUM2.NE.0.0) F=SUM1/SUM2
      IN=SUM2/NX
C
      RETURN
      END
...
C
  ...
  144 CONTINUE
      ...
      END
C**************************************************************************
      SUBROUTINE VA04A(NX,F1,F2,IN,X,E,N,F,ESCALE,IPRINT,ICON,
     +                 MAXIT)
C**************************************************************************
