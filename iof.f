...
C**************************************************************************
      IMPLICIT NONE
C
      INTEGER(KIND=4) I,HDR,IFILE,NXYZ(3),MO,NLABL
      INTEGER(KIND=4) SLEN,IFOL,IXLP,IYLP,IZLP
      INTEGER(KIND=4) I1(3),I13,I221
      PARAMETER (HDR=1024)
      INTEGER(KIND=1) BHDR(HDR)
      REAL(KIND=4) DMIN,DMAX,PSZ,AVDENS,SIGMA,DENSMAX
      REAL(KIND=4) DENSMIN,CELLI,CELLA,R77,R81,R85,R217
      REAL(KIND=4) R1,R5,RMODE,RMAX,RMIN,RMEAN,RRMS,R45
      REAL(KIND=8) DDRMS,DDMEAN
...
C
      IF (EX) THEN
        ...
        CALL CREAD(BHDR,0_8,INT(HDR,KIND=8),1_8,IFILE)
        CALL CCLOSE(IFILE)
C
...
C**************************************************************************
      IMPLICIT NONE
C
      INTEGER(KIND=4) I,HDR,IM,IFILE,MODE,NX,NY,NZ,MAPCRS(3)
      PARAMETER (HDR=1024,IM=200)
      INTEGER(KIND=4) NXYZ(3,IM),MO(IM),NSYMBT,NLABL(IM),BL
      INTEGER(KIND=4) LABBYT,IOMODE(IM),SLEN
      INTEGER(KIND=8) OFF(IM),RL(IM)
      REAL(KIND=4) DMIN(IM),DMAX(IM),PSIZE,PSZ(IM)
      REAL(KIND=8) DDRMS(IM),DDMEAN(IM)
      INTEGER(KIND=1) BHDR(HDR),SYM(80),BB
...
C
      ...
      DATA MAPCRS/1,2,3/
C
      COMMON /IODATA/NXYZ,MO,NLABL,IOMODE,OFF,RL,DMIN,
     +  DMAX,MAPCRS,PSZ,DDRMS,DDMEAN,LABEL,CF,BS,INAME
C
      SAVE /IODATA/
C**************************************************************************
C
...
C
     ...
     +          DDMEAN(IFILE),DDRMS(IFILE),MAPCRS,
...
C
     ...
     +          0.0,PSZ(IFILE),0.0D0,0.0D0,MAPCRS,
     +          NLABL(IFILE),LABEL(1,IFILE),1)
C
        INAME1=INAME(IFILE)(1:SLEN(INAME(IFILE)))//'.hed'
        CALL COPEN(INAME1,IFILE,IOMODE(IFILE))
C
        CALL CWRITE(BHDR,0_8,INT(HDR,KIND=8),1_8,IFILE)
C
        CALL CCLOSE(IFILE)
C
        INAME1=INAME(IFILE)(1:SLEN(INAME(IFILE)))//'.img'
...
C
     ...
     +          DMIN(IFILE),DMAX(IFILE),PSZ(IFILE),
     +          DDMEAN(IFILE),DDRMS(IFILE),MAPCRS,NSYMBT,
...
30      CONTINUE
        WRITE(*,*)
C
        IF (NSYMBT.NE.0) CALL CREAD(SYM,INT(HDR,KIND=8),
     +       INT(NSYMBT,KIND=8),1_8,IFILE)
C
        BL=2*MO(IFILE)
        IF (BL.EQ.0) BL=1
        RL(IFILE)=NXYZ(1,IFILE)*BL
...
C
     ...
     +          0.0,PSZ(IFILE),0.0D0,0.0D0,MAPCRS,NSYMBT,
...
C
        CALL CWRITE(BHDR,0_8,INT(HDR,KIND=8),1_8,IFILE)
C
        IF (NSYMBT.NE.0) CALL CWRITE(SYM,INT(HDR,KIND=8),
     +       INT(NSYMBT,KIND=8),1_8,IFILE)
C
        BL=2*MO(IFILE)
        IF (BL.EQ.0) BL=1
...
C
        IF (LABBYT.GT.HDR) THEN
          DO 50 I=1,LABBYT-HDR
            CALL CWRITE(BB,INT(HDR,KIND=8)+INT(I,KIND=8),1_8,1_8,IFILE)
50        CONTINUE 
        ENDIF
C
        RL(IFILE)=NXYZ(1,IFILE)*4
...
C
      IMPLICIT NONE
C
      INTEGER(KIND=4) I,IFILE,IM,IREC,MAPCRS(3)
      PARAMETER (IM=200)
      INTEGER(KIND=4) NXYZ(3,IM),MO(IM),NLABL(IM)
      INTEGER(KIND=4) IOMODE(IM),I2,I4
      INTEGER(KIND=8) OFF(IM),RL(IM)
      REAL(KIND=4) DMIN(IM),DMAX(IM),PSZ(IM),RR,R
      REAL(KIND=8) DDRMS(IM),DDMEAN(IM)
      INTEGER(KIND=2) II(2)
      INTEGER(KIND=1) LINE(*),BB(4)
...
C
      IF ((CF(IFILE).EQ.'I').OR.(CF(IFILE).EQ.'i')) THEN
C
        CALL CREAD(LINE,OFF(IFILE),RL(IFILE),INT(IREC,KIND=8),IFILE)
C
      ELSEIF ((CF(IFILE).EQ.'M').OR.(CF(IFILE).EQ.'m')) THEN
C
        CALL CREAD(LINE,OFF(IFILE),RL(IFILE),INT(IREC,KIND=8),IFILE)
...
C
      IMPLICIT NONE
C
      INTEGER(KIND=4) I,IFILE,IM,IREC,I4,I2,MAPCRS(3)
      PARAMETER (IM=200)
      INTEGER(KIND=4) NXYZ(3,IM),MO(IM),NLABL(IM)
      INTEGER(KIND=4) IOMODE(IM)
      INTEGER(KIND=8) OFF(IM),RL(IM)
      REAL(KIND=4) DMIN(IM),DMAX(IM),RR,PSZ(IM),R
      REAL(KIND=8) DDRMS(IM),DDMEAN(IM)
      INTEGER(KIND=2) II(2)
      INTEGER(KIND=1) LINE(*),BB(4)
      CHARACTER LABEL(10,IM)*80,CF(IM),INAME(IM)*200
...
C
      IF ((CF(IFILE).EQ.'I').OR.(CF(IFILE).EQ.'i')) THEN
C
        CALL CWRITE(LINE,OFF(IFILE),RL(IFILE),INT(IREC,KIND=8),IFILE)
C
      ELSEIF ((CF(IFILE).EQ.'M').OR.(CF(IFILE).EQ.'m')) THEN
C
        IF (BS(IFILE)) CALL BYTESWAP(LINE,RL(IFILE))
...
C
      IMPLICIT NONE
C
      INTEGER(KIND=4) HDR,IM,I,SLEN,MAPCRS(3)
      PARAMETER (HDR=1024,IM=200)
      INTEGER(KIND=4) IFILE,NXYZ(3,IM),MO(IM)
      INTEGER(KIND=8) OFF(IM),RL(IM)
      INTEGER(KIND=4) NLABL(IM),IOMODE(IM),LABBYT,NSYMBT,IMN
      REAL(KIND=4) DMIN(IM),DMAX(IM),PSZ(IM)
      REAL(KIND=8) DDRMS(IM),DDMEAN(IM)
      INTEGER(KIND=1) BHDR(HDR),SYM(80)
      CHARACTER LABEL(10,IM)*80,CF(IM),INAME(IM)*200
...
C
        DO 10 I=1,NXYZ(3,IFILE)
          IMN=I
          CALL CWRITE(BHDR,INT(I-1,KIND=8)*INT(HDR,KIND=8),
     +         INT(HDR,KIND=8),1_8,IFILE)
10      CONTINUE
C
        CALL CCLOSE(IFILE)
C
...
C**************************************************************************
      IMPLICIT NONE
C
      INTEGER(KIND=4) HDR,I,ID1,ID2,ID3,I4
      INTEGER(KIND=1) BUF(HDR),B
C
      DO 10 I=1,HDR/4
        I4=4*I
        ID1=I4-3
...
C**************************************************************************
     ...
     +                  DDMEAN,DDRMS,MAPCRS1,NLABL,LABEL,K)
C**************************************************************************
C
      IMPLICIT NONE
C
      INTEGER(KIND=4) I,K,HDR,IMN,IFOL,IERROR,NHFR,NMONTH,IZLP
      PARAMETER (HDR=1024)
      INTEGER(KIND=4) NXYZ(3),MODE,EXTRA1(7),EXTRA2(53),MAPCRS1(3)
      INTEGER(KIND=4) NLABL,NDAY,NYEAR,NHOUR,NMINUT,NXYZSTART(3)
      INTEGER(KIND=4) NSEC,NPIX2,NPIXEL,IXLP,IYLP,REALTYPE
      INTEGER(KIND=4) IXOLD,IYOLD,COMPLEX,MAPCRS(3),ISPG
      INTEGER(KIND=4) MXYZ(3),REALTYPE1,EXTRA3(74),NMON(12)
      REAL(KIND=4) PSIZE,DMIN,DMAX,AVDENS,SIGMA,VARIAN,OLDAVD
      REAL(KIND=4) DENSMAX,DENSMIN,CALPHA,CBETA,CGAMMA,CELLA(3)
      REAL(KIND=4) RESOLXYZ(3)
      REAL(KIND=8) DDMEAN,DDRMS
      INTEGER(KIND=1) HDR1(HDR),IHDR(HDR)
...
C**************************************************************************
     ...
     +       DDMEAN,DDRMS,MAPCRS1,NSYMBT,NLABL,LABEL,BS,K)
C**************************************************************************
C
      IMPLICIT NONE
C
      INTEGER(KIND=4) MXYZ(3),MAPCRS(3),I,K,I1(3),I13,HDR
      PARAMETER (HDR=1024)
      INTEGER(KIND=4) NXYZ(3),NXYZSTART(3),I93,I221,MODE
      INTEGER(KIND=4) ISPG,NSYMBT,EXTRA(25),NLABL,MAPCRS1(3)
      REAL(KIND=4) CELLA(3),CELLB(3),R77,R81,R85,PSIZE
      REAL(KIND=4) ORIGIN(3),DMIN,DMAX,R217
      REAL(KIND=8) DDMEAN,DDRMS
      INTEGER(KIND=1) HDR1(HDR),MHDR(HDR),MACHST(4),MACHST1(4)
...
C
        ...
        DMIN=R77
        DMAX=R81
C        PSIZE=CELLA(1)/I1(1)
        PSIZE=CELLA(1)/MXYZ(1)
        DDMEAN=DBLE(R85)
        DDRMS=DBLE(R217)
        MAPCRS1(1)=MAPCRS(1)
        MAPCRS1(2)=MAPCRS(2)
...
C
      IMPLICIT NONE
C
      INTEGER(KIND=4) NXYZ(3),LABBYT,LENBYT,LABREC,I,K,MODE
      INTEGER(KIND=4) NLABL,HDR
      PARAMETER (HDR=1024)
      REAL(KIND=4) RMAMI,RLABREC,RANGLE,RLABBYT,RLENBYT
      REAL(KIND=4) RSTACK,RMAXIM,RMGNUM,RMODE,RLASTINDX,RANGLE2
      REAL(KIND=4) PARA(7),ANG(6),JM(174),RREC,PSIZE
      REAL(KIND=4) R1,R5,RMAX,RMIN,RMEAN,RRMS,R45,R161,R385
      REAL(KIND=4) R449,R465,DMIN,DMAX
      REAL(KIND=8) DDMEAN,DDRMS
      INTEGER(KIND=1) HDR1(HDR),SHDR(HDR)
...
C
      IMPLICIT NONE
C
      INTEGER(KIND=4) I,A0,A1,A2,A3,REALTYPE
      PARAMETER (A0=48,A1=49,A2=50,A3=51)
C
      COMMON /ENDIAN/I
C**************************************************************************
...
C
C**************************************************************************
      INTEGER(KIND=4) FUNCTION SLEN(STRG)
C**************************************************************************
C
      IMPLICIT NONE
C
